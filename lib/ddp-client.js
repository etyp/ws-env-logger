...
class DDPClient extends EventEmitter{
  ...
  _send(data) {
    var self = this;
    self.log(`SENDING: ${EJSON.stringify(data)}`);
    self.socket.send(
  ...
  _message(data) {
    ...
    if (!data.msg) {
      ...
      self.log(`[_message] connected`);
      self.session = data.session;
      self.emit("connected");

    // method result
    } else if (data.msg === "result") {
      self.log(`[_message] result`);
